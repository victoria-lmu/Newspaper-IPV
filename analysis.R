...
# by gender_authors
q2_faceted2 <- articles %>%
  select(gender_author,
         incident_not_in_title, ipv_not_in_title, ipv_not_in_text,
         blame_victim, blame_perp,
         attack_quarrel, murder_tragedy, language_passive) %>%
  filter(!(gender_author == "NA")) %>%
  ...
  mutate(variable = factor(variable, levels = c("language_passive",
                                                ...
                                                "murder_tragedy",
                                                "attack_quarrel")))
ggplot(q2_faceted2, aes(x = variable, fill = factor(value))) +
  geom_bar(position = "fill", width = 0.8) +
  coord_flip() +
  # order so that left-leaning newspapers in left column, right in right
  ...
  theme(plot.title = element_blank(),
        ...
        panel.grid.major = element_line(colour = "lightgrey"),
        panel.grid.minor = element_line(colour = "lightgrey", linetype = "dashed"))
ggsave("ling_framing_author.png", path = "./plots", width = 12.5, height = 5)



# by perp_migrant: among those where ethnicity mentioned (i.e. filter out migr_perp = "NA" <=> ethn_migr = "No")
q2_migr <- articles %>%
  select(migr_perp,
         incident_not_in_title, ipv_not_in_title, ipv_not_in_text,
         blame_victim, blame_perp,
         attack_quarrel, murder_tragedy, language_passive) %>%
  filter(!(migr_perp == "NA")) %>%
  pivot_longer(cols = -migr_perp, names_to = "variable", values_to = "value") %>%
  mutate(variable = factor(variable, levels = c("language_passive",
                                                "blame_perp",
                                                "blame_victim",
                                                "incident_not_in_title",
                                                ...
                                                "murder_tragedy",
                                                "attack_quarrel")))
ggplot(q2_migr, aes(x = variable, fill = factor(value))) +
  geom_bar(position = "fill", width = 0.8) +
  coord_flip() +
  facet_wrap(~migr_perp,
             labeller = as_labeller(c("1" = "German Nationality",
                                      "2" = "Migrant/No German Nationality"))) + 
  labs(y = "Proportion",
       title = "Linguistic Framing by Perpetrator's Nationality (?)") +
  scale_fill_manual(values = c("lightblue", "lightblue4", "gray90"),
                    labels = c("False", "True")) +
  scale_x_discrete(labels = c(
    "murder_tragedy" = "Murder as 'tragedy'",
  ...
  theme(plot.title = element_blank(),
        ...
        panel.grid.major = element_line(colour = "lightgrey"),
        panel.grid.minor = element_line(colour = "lightgrey", linetype = "dashed"))
ggsave("ling_framing_perp.png", path = "./plots", width = 9, height = 5)
...
q3_perp <- articles %>%
  ...
  pivot_longer(cols = -migr_perp, names_to = "variable", values_to = "value")  %>%
  mutate(fill_key = paste(variable, value, sep = "_"))  # combined mapping key
ggplot(q3_perp, aes(x = variable, fill = fill_key)) +
  geom_bar(position = "fill", width = 0.7) +
  facet_grid(~ migr_perp,
             labeller = as_labeller(c("1" = "German Nationality",
                                      "2" = "Migrant/No German Nationality"))) +
  labs(y = "Proportion",
       title = "Missing Contextualisation by Perpetrator's Nationality (?)") +
  scale_fill_manual(values = colour_map,
                    labels = c("episodic_framing_1" = "No episodic framing",
                               "episodic_framing_2" = "Episodic framing",
                               "struct_context_no_1" = "Similar past incidents mentioned",
  ...
  theme(plot.title = element_blank(),
        ...
        panel.grid.major = element_line(colour = "lightgrey"),
        panel.grid.minor = element_line(colour = "lightgrey", linetype = "dashed"))
ggsave("context_faceted_perp.png", path = "./plots", width = 9.5, height = 5)
